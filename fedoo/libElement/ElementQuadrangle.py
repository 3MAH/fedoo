...
class elementQuadrangle(element2D):
    def __init__(self,nb_gp): 
        ...
        if nb_gp == 1: #ordre exact 1
            ...
            assert 0, "Number of gauss points "+str(nb_gp)+" unavailable for quadrangle element"  
...
class quad4(elementQuadrangle):
    def __init__(self, nb_gp=4, **kargs):
        self.xi_nd =  np.c_[[-1. , 1., 1., -1.],\
                            [-1. , -1., 1., 1.]]
        self.nb_gp = nb_gp
        elementQuadrangle.__init__(self, nb_gp)
...
class quad8(elementQuadrangle):
    def __init__(self, nb_gp=9, **kargs):
