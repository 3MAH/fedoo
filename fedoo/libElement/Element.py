...
class element:              
    ...
    def ComputeDetJacobian(self,vec_x, vec_xi):
        ...
        self.JacobianMatrix = np.moveaxis([np.dot(dnn,vec_x) for dnn in dnn_xi], 2,0) #shape = (vec_x.shape[0] = Nel, len(vec_xi)=nb_pg, nb_dir_derivative, vec_x.shape[2] = dim)
...
#            self.inverseJacobian = [linalg.inv(J) for J in self.JacobianMatrix]
            ...
            self.inverseJacobian = np.matmul(JT , linalg.inv(np.matmul(J,JT)))    #inverseJacobian.shape = (Nel,len(vec_xi)=nb_pg, dim:vec_x.shape[-1], dim:vec_xi.shape[-1])            
