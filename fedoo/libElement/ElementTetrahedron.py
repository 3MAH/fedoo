...
class elementTetrahedron(element):
    def __init__(self,nb_gp):                 
        ...
        if nb_gp == 1:
            xi = np.c_[[1./4]] ; eta = np.c_[[1./4]] ; zeta = np.c_[[1./4]]
            w_pg = np.array([1./6])                                
        elif nb_gp == 4:
            a = 0.1381966011250105 ; b = 0.5854101966249685
            xi =  np.c_[[a, a, a, b]] ; eta = np.c_[[a, a, b, a]] ; zeta = np.c_[[a, b, a, a]]
            w_pg = np.array([1./24, 1./24, 1./24, 1./24])
        elif nb_gp == 5:
            a = 0.25 ; b = 0.16666666666666666 ; c = 0.5
            xi =  np.c_[[a, b, b, b, c]] ; eta = np.c_[[a, b, b, c, b]] ; zeta = np.c_[[a, b, c, b, b]]
            w_pg = np.array([-2./15, 3./40, 3./40, 3./40, 3./40])
        elif nb_gp == 15:
            ...
            xi =  np.c_[[a, b1, b1, b1, c1, b2, b2, b2, c2, d, d, e, d, e, e]] ; eta = np.c_[[a, b1, b1, c1, b1, b2, b2, c2, b2, d, e, d, e, d, e]] ; zeta = np.c_[[a, b1, c1, b1, b1, b2, c2, b2, b2, e, d, d, e, e, d]]
            f1 = 0.011511367871045397 ; f2 = 0.01198951396316977
            w_pg = np.array([8./405, f1, f1, f1, f1, f2, f2, f2, f2, 5./567, 5./567, 5./567, 5./567, 5./567, 5./567])
        elif nb_gp == 0:
            self.xi_pg = self.xi_nd #if nb_gp == 0, we take the position of the nodes
        else:
            assert 0, "Number of gauss points "+str(nb_gp)+" unavailable for tetrahedron element"  
...
class tet4(elementTetrahedron):
    def __init__(self, nb_gp=4, **kargs):
        self.xi_nd = np.c_[[0. , 0. , 0. , 1.],\
                           ...
                           [0. , 1. , 0. , 0.]]
        self.nb_gp = nb_gp
        elementTetrahedron.__init__(self,nb_gp)
...
class tet10(elementTetrahedron):
    def __init__(self, nb_gp=15, **kargs):
    ...
    def ShapeFunction(self, vec_xi):
        xi = vec_xi[:,0] ; eta = vec_xi[:,1]; zeta = vec_xi[:,2]
        return np.c_[eta*(2*eta-1), zeta*(2*zeta-1), (1-xi-eta-zeta)*(1-2*xi-2*eta-2*zeta), xi*(2*xi-1), 4*eta*zeta, 4*zeta*(1-xi-eta-zeta), 4*eta*(1-xi-eta-zeta), 4*xi*eta, 4*xi*zeta, 4*xi*(1-xi-eta-zeta)]    
    def ShapeFunctionDerivative(self, vec_xi):
        vec_m = [1-xi[0]-xi[1]-xi[2] for xi in vec_xi]
        return [ np.array([ [ 0.        , 0.        , 1-4*m, -1+4*xi[0], 0.     , -4*xi[2]   , -4*xi[1]   , 4*xi[1], 4*xi[2], 4*(m-xi[0]) ] , \
