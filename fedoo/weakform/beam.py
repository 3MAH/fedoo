            ...
            if self.nlgeom:
                ...
                if self.space.ndim == 2:               
                    ...
                    if '_ElementVectors' in assembly.sv:
                        ...
                        initial_element_vectors = assembly.mesh.nodes[mesh.elements[:,1]]-assembly.mesh.nodes[mesh.elements[:,0]]
                    ...
                    delta_angle = np.arctan2( 
                        element_vectors_old[:,0]*element_vectors[:,1]-element_vectors_old[:,1]*element_vectors[:,0], 
                        element_vectors_old[:,0]*element_vectors[:,0]+element_vectors_old[:,1]*element_vectors[:,1]
                        )
                    ...
                    dof_local[var*n_dof_per_var:(var+1)*n_dof_per_var] = np.hstack([rot1.T,rot2.T]).ravel()
                    
                    var = self.space.variable_rank('DispX')
                    dof_local[var*n_dof_per_var+mesh.n_elements:(var+1)*n_dof_per_var] = u2x                                        
                    ...
                    rigid_rotmat_trial = Rotation.from_rotvec(delta_rotvec).as_matrix().transpose(0,2,1) @ assembly.sv['RigidRotationMat']
                    ...
                    #compute u2, rot1 and rot2, ie the rot dof at node 1 and 2
                    #u1 = 0 as the first node is the center of the local bases
                    nodes_rotmat = Rotation.from_rotvec(delta_rotvec_nodes.T).as_matrix() @ assembly.sv['_NodesRotationMatrix']
                    
                    rot1 = Rotation.from_matrix(rigid_rotmat @ nodes_rotmat[mesh.elements[:,0]]).as_rotvec()
                    ...
                    dof_local[var*n_dof_per_var:(var+3)*n_dof_per_var] = np.hstack([rot1.T,rot2.T]).ravel()
                ...
                assembly.sv['BeamStrain'] = [0 if Ke[i] is 0 or op is 0 else assembly.get_gp_results(op, dof) 
                                              for i,op in enumerate(op_beam_strain)]
