...
class ViscoElasticComposites(Mechanical3D):
    ...
    def get_stress(self, localFrame=None): # methode virtuel
        ...
        if isinstance(EL, (float,int,np.number)): H = sp.empty((6,6))
        elif isinstance(EL,(sp.ndarray,list)): H = sp.zeros((6,6,len(EL)))
        else: H = sp.zeros((6,6), dtype='object')

        d1,d2,d3,d4,d5,d6 = self.__DamageVariables
        
        StrainRateEffect = [(StrainRate[i] >= RefStrainRate) * np.log(StrainRate[i]/RefStrainRate) for i in range(6)]
                
        nuTL = nuLT*ET/EL 
        k = 1-nuTT**2 - 2*nuLT*nuTL  - 2*nuLT*nuTT*nuTL
        H[0,0] = ((1-d1)*EL*(1 + CL*StrainRateEffect[0]))*(1-nuTT**2)/k 
        H[1,1] = ((1-d2)*ET*(1 + CT*StrainRateEffect[1]))*(1-nuLT*nuTL)/k
