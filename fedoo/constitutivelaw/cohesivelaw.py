...
class CohesiveLaw(Spring):
    ...
    def __init__(self, GIc=0.3, SImax = 60, KI = 1e4, GIIc = 1.6, SIImax=None, KII=5e4, axis = 2, name =""):
        ...
        self.parameters = {'GIc':GIc, 'SImax':SImax, 'KI':KI, 'GIIc':GIIc, 'SIImax':SIImax, 'KII':KII, 'axis':axis}                          
    
    
    def initialize(self, assembly, pb):
        assembly.sv['InterfaceStress'] = 0 #Interface Stress
        assembly.sv['DamageVariable'] = 0 #damage variable
        assembly.sv['DamageVariableOpening'] = 0 # DamageVariableOpening is used for the opening mode (mode I). It is equal to DamageVariable in traction and equal to 0 in compression (soft contact law)    
        assembly.sv['DamageVariableIrreversible'] = 0 #irreversible damage variable used for time evolution 
    ...
    def update_irreversible_damage(self, assembly):
        if assembly.sv['DamageVariable'] is 0: assembly.sv['DamageVariableIrreversible'] = 0
        else: assembly.sv['DamageVariableIrreversible'] = assembly.sv['DamageVariable'].copy()
    ...
    def update_damage(self, assembly, U, irreversible = False, type_data = 'PG'):         
        ...
        else: delta = [assembly.get_gp_results(op, U) for op in op_delta]            
        
        self._update_damage(assembly, delta)
        
        if irreversible: assembly.sv['DamageVariableIrreversible'] = assembly.sv['DamageVariable'].copy()
...
#        beta = 0*delta_n
        ...
        tm[ind_traction]= (2*((1+ beta)**2)/t0[ind_traction]) * ((((self.parameters['KI']/self.parameters['GIc'])**alpha) + \
        ...
        if (assembly.sv['DamageVariableIrreversible'] is 0): assembly.sv['DamageVariable'] = d #I don't know why assembly.sv['DamageVariable'] = d end up in bads values
        else:
            assembly.sv['DamageVariable'] = np.max([assembly.sv['DamageVariableIrreversible'], d], axis = 0)                                                                                                      
                                                                                           
        assembly.sv['DamageVariableOpening'] = (delta_n > 0)*assembly.sv['DamageVariable'] #for opening the damage in considered to 0 when the relative displacement is negative (conctact)
