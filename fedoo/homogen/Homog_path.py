...
if USE_SIMCOON:
    ...
    def get_resultsUnitCell(
        ...
        BlocksCyclesSteps_df = pd.DataFrame(
            np.array(Res[0]), columns=BlocksCyclesSteps_columns
        )
        ...
        if Tangent_bool:
            ...
            TangentMatrix_df = pd.DataFrame(
                np.array(Res[4]), columns=TangentMatrix_columns
            )
            ...
            Results_df = pd.concat(
                [BlocksCyclesSteps_df, MeanStrain_df, MeanStress_df, Wm_df], axis=1
            )
