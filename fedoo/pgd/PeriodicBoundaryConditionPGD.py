...
def DefinePeriodicBoundaryCondition(mesh, NodeEps, VarEps, dim='3D', tol=1e-8, problem = None):
    ...
    if dim == 2:                       
        ...
        mesh.add_node_set([np.full_like(right, NodeEps[0])], [numGeometricMesh], name ='EpsXX_right')
        mesh.add_node_set([np.full_like(right, NodeEps[2])], [numGeometricMesh], name ='EpsXY_right')
        mesh.add_node_set([np.full_like(top,NodeEps[2])],    [numGeometricMesh], name ='EpsXY_top')
        mesh.add_node_set([np.full_like(top,NodeEps[1])],    [numGeometricMesh], name ='EpsYY_top')        
        mesh.add_node_set([np.full_like(corner_lt,NodeEps[0])], [numGeometricMesh], name ='EpsXX_corner')
        mesh.add_node_set([np.full_like(corner_lt,NodeEps[1])], [numGeometricMesh], name ='EpsYY_corner')
        ...
        mesh.add_node_set([np.full_like(right, NodeEps[0])], [numGeometricMesh], name ='EpsXX_right')
        mesh.add_node_set([np.full_like(right, NodeEps[3])], [numGeometricMesh], name ='EpsXY_right')
        mesh.add_node_set([np.full_like(right, NodeEps[4])], [numGeometricMesh], name ='EpsXZ_right')
        mesh.add_node_set([np.full_like(top,NodeEps[3])],    [numGeometricMesh], name ='EpsXY_top')
        mesh.add_node_set([np.full_like(top,NodeEps[1])],    [numGeometricMesh], name ='EpsYY_top')
        mesh.add_node_set([np.full_like(top,NodeEps[5])],    [numGeometricMesh], name ='EpsYZ_top')
        mesh.add_node_set([np.full_like(front,NodeEps[4])],  [numGeometricMesh], name ='EpsXZ_front')
        mesh.add_node_set([np.full_like(front,NodeEps[5])],  [numGeometricMesh], name ='EpsYZ_front')
        mesh.add_node_set([np.full_like(front,NodeEps[2])],  [numGeometricMesh], name ='EpsZZ_front')

        mesh.add_node_set([np.full_like(edge_lt,NodeEps[0])],[numGeometricMesh], name ='EpsXX_edge_lt')
        mesh.add_node_set([np.full_like(edge_lt,NodeEps[1])],[numGeometricMesh], name ='EpsYY_edge_lt')
        mesh.add_node_set([np.full_like(edge_lt,NodeEps[3])],[numGeometricMesh], name ='EpsXY_edge_lt')
        mesh.add_node_set([np.full_like(edge_lt,NodeEps[4])],[numGeometricMesh], name ='EpsXZ_edge_lt')
        mesh.add_node_set([np.full_like(edge_lt,NodeEps[5])],[numGeometricMesh], name ='EpsYZ_edge_lt')
    
        mesh.add_node_set([np.full_like(edge_bf,NodeEps[1])],[numGeometricMesh], name ='EpsYY_edge_bf')
        mesh.add_node_set([np.full_like(edge_bf,NodeEps[2])],[numGeometricMesh], name ='EpsZZ_edge_bf')
        mesh.add_node_set([np.full_like(edge_bf,NodeEps[3])],[numGeometricMesh], name ='EpsXY_edge_bf')
        mesh.add_node_set([np.full_like(edge_bf,NodeEps[4])],[numGeometricMesh], name ='EpsXZ_edge_bf')
        mesh.add_node_set([np.full_like(edge_bf,NodeEps[5])],[numGeometricMesh], name ='EpsYZ_edge_bf')

        mesh.add_node_set([np.full_like(edge_ld,NodeEps[0])],[numGeometricMesh], name ='EpsXX_edge_ld')
        mesh.add_node_set([np.full_like(edge_ld,NodeEps[2])],[numGeometricMesh], name ='EpsZZ_edge_ld')
        mesh.add_node_set([np.full_like(edge_ld,NodeEps[3])],[numGeometricMesh], name ='EpsXY_edge_ld')
        mesh.add_node_set([np.full_like(edge_ld,NodeEps[4])],[numGeometricMesh], name ='EpsXZ_edge_ld')
        mesh.add_node_set([np.full_like(edge_ld,NodeEps[5])],[numGeometricMesh], name ='EpsYZ_edge_ld')

        mesh.add_node_set([np.full_like(corner_rbd,NodeEps[0])], [numGeometricMesh], name ='EpsXX_corner')
        mesh.add_node_set([np.full_like(corner_rbd,NodeEps[1])], [numGeometricMesh], name ='EpsYY_corner')
        mesh.add_node_set([np.full_like(corner_rbd,NodeEps[2])], [numGeometricMesh], name ='EpsZZ_corner')
        mesh.add_node_set([np.full_like(corner_rbd,NodeEps[3])], [numGeometricMesh], name ='EpsXY_corner')
        mesh.add_node_set([np.full_like(corner_rbd,NodeEps[4])], [numGeometricMesh], name ='EpsXZ_corner')
        mesh.add_node_set([np.full_like(corner_rbd,NodeEps[5])], [numGeometricMesh], name ='EpsYZ_corner')
