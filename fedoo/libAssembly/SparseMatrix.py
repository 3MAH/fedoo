...
class _BlocSparseOld():
    ...
    def addToBlocATB(self, A, B, coef, rowBloc, colBloc):
        ...
        if self.data[rowBloc][colBloc] is 0:             
            ...
            # self.data[rowBloc][colBloc] += (coef * A.data.reshape(-1,NnzColPerRowA,1)).reshape(nb_pg,-1,NnzColPerRowA).transpose((1,2,0)) @ B.data.reshape(nb_pg,-1,NnzColPerRowB).transpose(1,0,2) #at element PG we build a nbNode x nbNode matrix
            temp = A.data.reshape(-1,NnzColPerRowA,1) @ B.data.reshape(-1,1,NnzColPerRowB) #at each PG we build a nbNode x nbNode matrix
            self.data[rowBloc][colBloc] += coef * temp
