...
mesh = fd.mesh.extrude(profil,
                       100, #extrusion length,
                       11) #n_nodes
mesh.nodes = mesh.nodes[:,[2,1,0]] #switch axis to put the extrusion direction along the X axis
...
left_bottom = np.intersect1d(mesh.find_nodes('X', mesh.bounding_box.xmin),
                      bottom)
right_bottom = np.intersect1d(mesh.find_nodes('X', mesh.bounding_box.xmax),
                      bottom)
center_top = np.intersect1d(mesh.find_nodes('X', mesh.bounding_box.center[0]),
                      top)
