...
mesh = fd.mesh.rectangle_mesh(nx=41, ny=21, x_min=0, x_max=L, y_min=0, y_max=h, elm_type = 'quad4', name = "Domain")
...
tmax = 1
pb.bc.add('Dirichlet',bottom_left, 'Disp', 0,)
...
while pb.time < tmax - err_num:                 
    pb.assembly.set_start(pb) #set the tangent matrix to the elastic and make the global matrices assembly    
    if ii != 0: pb.save_results(ii) #Save results            
    ...
    for subiter in range(max_subiter): #newton-raphson iterations
        ...
        #--------------- Solve --------------------------------------------------------        
        pb.assembly.update(pb) #update the initial stress and tangent matrix from the constitutive law and make the globel matrix and vector assemblies
    ...
    if not(convergence):
        raise NameError('Newton Raphson iteration has not converged (err: {:.5f})- Reduce the time step.'.format(norm_res))   

    pb.time = pb.time + pb.dtime #update time value 
    ii += 1
    
    pb.assembly.update(pb) #update the stress and tangent matrix from the constitutive law and make the globel matrix and vector assemblies

    
    print('Iter {} - Time: {:.5f} - NR iter: {} - Err: {:.5f}'.format(ii, pb.time, subiter, norm_res))
