...
plate_elm_type = 'p'+geomElementType #plate interpolation. Same as geom interpolation in local element coordinate (change of basis)
...
mesh = fd.mesh.rectangle_mesh(201,21,0,L,-h/2,h/2, geomElementType, name='plate', ndim = 3)
...
res = pb.add_output('results/simple_plate', assemb, ['Disp', 'Rot', 'Stress', 'Strain'], output_type='Node', file_format ='vtk', position = +1) 
...
print('Numerical deflection with shell model: ', pb.get_disp('DispZ')[node_right_center])

if save_results: 
    pb.save_results() #save in vtk

z, StressDistribution = fd.ConstitutiveLaw.get_all()['PlateSection'].GetStressDistribution(assemb,20)
