...
eq_steady = D[0].virtual * (-P+2*mu*D[0]) + D[1].virtual * (-P+2*mu*D[1]) + 0.5*D[2].virtual * D[2]
...
class TimeDerivative(fd.core.weakform.WeakFormBase):
    def __init__(self, rho, name = None, space = None):            
        ...
        self.__dtime = 0
        self.rho = rho
        self.assembly_options['mat_lumping'] = True #use mat lumping for time derivative
                
    def initialize(self, assembly, pb, t0 = 0.):
        if not(np.isscalar(pb.get_dof_solution())):
            self.__v_start = assembly.convert_data(pb.get_dof_solution('V'), convert_from='Node', convert_to='GaussPoint')
            self.__v = self.__v_start
        else: 
            self.__v_start = self.__v = 0

    def update(self, assembly, pb, dtime):
        self.__v = assembly.convert_data(pb.get_dof_solution('V'), convert_from='Node', convert_to='GaussPoint')
    ...
    def get_weak_equation(self, mesh=None):      
        ...
        if self.__dtime != 0:
            return 1/self.__dtime * self.rho * (op_vx.virtual * op_vx + op_vx.virtual *(self.__v[0] - self.__v_start[0])) +\
                   1/self.__dtime * self.rho * (op_vy.virtual * op_vy + op_vy.virtual *(self.__v[1] - self.__v_start[1]))
        else:            
            return 0
...
class Convection(fd.core.weakform.WeakFormBase):
    ...
    def get_weak_equation(self, mesh=None):      
        ...
        Uold_grad_U = [Vx_old * dVx_dX + Vy_old * dVx_dY, Vx_old * dVy_dX + Vy_old * dVy_dY] #to multiply by [Vx.virtual, Vy.virtual]
...
wf2 = fd.core.weakform.WeakFormSum([TimeDerivative(rho, 'dVdt'), Convection(rho,'convection')]) 
wf = wf1+wf2

# mesh = fd.mesh.rectangle_mesh(elm_type = "tri3")
mesh1 = fd.mesh.hole_plate_mesh(radius = 20, elm_type = 'tri3',sym = False)
mesh2 = fd.mesh.rectangle_mesh(nx=41, ny=21, x_min=50, x_max=250,y_min=-50,y_max=50,elm_type='tri3')
