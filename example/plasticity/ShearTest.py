...
Mesh.box_mesh(Nx=11, Ny=11, Nz=11, x_min=0, x_max=L, y_min=0, y_max=h, z_min = 0, z_max = w, ElementShape = 'hex20', name = meshname)
...
Assembly.Create("ConstitutiveLaw", meshname, name="Assembling", MeshChange = False, nb_pg = 8)     #uses MeshChange=True when the mesh change during the time

Problem.NonLinearStatic("Assembling")
# Problem.SetSolver('cg', precond = True)
Problem.SetNewtonRaphsonErrorCriterion("Displacement", err0 = 1, tol = 5e-3, max_subiter = 20)
...
res = np.load(res_name+'_{}.npz'.format(19))
