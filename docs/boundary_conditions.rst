...
The constraint uses one Lagrange multiplier per variable, producing an
indefinite saddle-point system; use a direct solver. The zero-mean condition
provides a canonical periodic fluctuation field while retaining the same stress
field as a single-node pin.
